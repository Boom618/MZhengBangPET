 ...
 * 生产入库 已办详情
 */
class ProductDoneDetailAdapter(context: Context, layoutId: Int, datas: List<ProductDetails.ListBean>) : CommonAdapter<ProductDetails.ListBean>(context, layoutId, datas) {

    override fun convert(holder: ViewHolder, list: ProductDetails.ListBean, position: Int) {
        holder.setText(R.id.tv_document_no, "生产订单：${list.sapOrderNo}")
                .setText(R.id.tv_material_name, "辅料名称：${list.goodsName}")
                .setText(R.id.tv_number, "入库数量：${list.number}")
                .setText(R.id.tv_solubility, "批次：")
